...
slide_generator_app = agents.create(display_name="Slide Generator",model=MODEL, instruction="You create clear, concise slide decks.  Each slide has a title, a short paragraph (max 3 sentences), and bullet points. Separate slides with '=== slide ==='.")
...
# All the fancy code around progress bars is suggested by Gemini
...
def create_pdf(slide_data, pdf_file):
    ...
    for slide in slide_data:
        ...
        if title:  # Add title if it exists
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            query_input = gr.Textbox(label="Presentation Topic", placeholder="Enter your topic here")
            refinement_rounds_input = gr.Slider(minimum=1, maximum=5, step=1, value=2, label="Refinement Rounds")
